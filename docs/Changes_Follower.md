...
[POST `_changes` API](https://github.com/IBM/cloudant-go-sdk/tree/v0.10.18/examples#postchanges) instead of the follower.
...
In these use-cases use the SDK's [POST `_changes` API](https://github.com/IBM/cloudant-go-sdk/tree/v0.10.18/examples#postchanges)
...
```go
latestSeq, err := follower.LatestSequenceFrom(checkpointSequenceId)
```
